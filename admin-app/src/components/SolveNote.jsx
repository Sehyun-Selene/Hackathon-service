...
// key 를 바꿔 이 칸을 새로 만들기 때문에, 앞 호출에 적던 글이 따라오지
// 않습니다.
export default function SolveNote({ value, onChange, id }) {
  // 처음에는 한 줄만 보입니다. 두 줄짜리 칸이 늘 펼쳐져 있으면 '적어야
  // 하는 것'처럼 보여서, 적을 사람만 열어 쓰게 합니다.
  const [open, setOpen] = useState(() => Boolean(value))

  if (!open) {
    return (
      <button type="button" className="solve-note-open" onClick={() => setOpen(true)}>
