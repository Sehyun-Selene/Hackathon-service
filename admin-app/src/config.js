...
// ---------------------------------------------------------------
// ⚠️ 테스트용 조기 개방 — 해커톤 전에 null 로 되돌리세요.
//
// 이 값이 있으면 주문 화면이 이 시각부터 열립니다. 마스터 메이트들이
// 해커톤 전에 실제 화면을 눌러 보게 하려는 것입니다.
//
...
export const ORDER_PREVIEW_FROM = '2026-09-18T16:00:00'
...
//
//   - leagues : 이 탭을 보여줄 리그. 타임테이블은 리그마다 일정이 달라
//               두 장을 따로 두고 각자에게 한 장씩만 보입니다. 탭 이름과
//               아이콘은 같아서, 참가자에게는 그냥 '타임테이블' 한 칸입니다.
...
export const IMAGE_BOARDS = [
  ...
  {
    // 필드리그와 일정이 달라 이미지가 따로입니다. 두 탭이 동시에 보이는
    // 일은 없습니다 — imageBoardsFor 가 리그로 걸러 한 장만 내보냅니다.
    id: 'timetable-dev',
    label: '타임테이블',
    icon: '🗓️',
    title: '개발자리그 타임테이블',
    src: './board/timetable-dev.png?v=1',
    alt: '개발자리그 타임테이블 — DAY1 9/21, DAY2 9/22 시간별 일정',
    leagues: ['dev'],
  },
  {
    id: 'journey',
...
// ---------------------------------------------------------------
// 1-3. 바로가기 탭 (이벤트 안내 · 해커톤 플랫폼 · 더밀크 구독권)
//
//   해커톤 중에 앱 밖으로 나가야 하는 곳을 한자리에 모읍니다. 탭 이름을
//   '이벤트'가 아니라 '바로가기'로 둔 것은, 해커톤 플랫폼이 즐길거리가
//   아니라 제출 때문에 반드시 들러야 하는 곳이기 때문입니다.
//
//   ⚠️ EVENT_GUIDE_URL 은 노션 원본이 아니라 공개 게시본(오피)입니다.
//      원본(gsholdings.notion.site/…)은 두 가지 이유로 못 씁니다:
//        ① 웹에 게시된 상태가 아니라, GS그룹 노션 계정이 없는 참가자는
//           로그인 벽을 만납니다 — 계열사·외부 인원이 섞여 있습니다.
//        ② X-Frame-Options: SAMEORIGIN 이라 앱 안에 띄울 수도 없습니다.
//      오피 주소는 둘 다 해당하지 않습니다. 노션을 고치면 이 주소에도
//      그대로 반영되므로, 내용이 바뀌어도 앱을 다시 배포할 필요가 없습니다.
// ---------------------------------------------------------------
export const EVENT_GUIDE_URL = 'https://52g.oopy.io/event_filed'

// 두 리그 공통 이벤트를 한 장에 담은 포스터. 안내 이미지 탭과 같은 방식으로
// 폭에 맞춰 보여주고, 눌러서 원본을 크게 엽니다.
export const EVENT_POSTER = {
  src: './board/event-common.jpg?v=1',
  alt:
    '필드리그 & 개발자리그 공통 이벤트 — 01 Campfire Kit 소개, 02 PLAI MOVIE FESTIVAL, ' +
    '03 ENTER:PLAI, 04 RE:PLAI, 05 SNS 인증',
}

// 개발자리그에서만 열리는 이벤트. 포스터가 01~05를 덮으므로 번호를 이어
// 씁니다 — 노션과 번호가 같아야 현장에서 "6번 이벤트"라는 말이 통합니다.
// 개발자리그 참가자에게만 보여줍니다.
export const DEV_LEAGUE_EVENTS = [
  { no: '06', title: '봉인 해제 최장 구동 인증', desc: '최장 구동 기록을 함께 인증하세요!' },
  { no: '07', title: '우수팀', desc: '맥 미니의 주인공에 도전하세요!' },
]

// 체크아웃 설문 — 해커톤이 끝날 때 남기는 설문입니다.
//
//   바로가기 탭 맨 아래에 섭니다. 필드리그에만 — 개발자리그는 설문이
...
// 그 팀에게 보여줄 체크아웃 설문 (해당 없으면 null)
export function checkoutSurveyFor(teamId) {
  if (!CHECKOUT_SURVEY.url) return null
  const league = leagueOf(teamId)?.id
  return CHECKOUT_SURVEY.leagues.includes(league) ? CHECKOUT_SURVEY : null
}

// 앱 밖 사이트. 둘 다 새 탭에서 엽니다 — 앱 안에 가두면 참가자가 주문·호출로
// 돌아올 길을 잃습니다.
//
// 한 묶음으로 두지 않았습니다. 성격이 다릅니다:
//   플랫폼은 제출 때문에 반드시 쓰는 도구라 화면 맨 위에 소제목 없이 홀로
//   놓습니다 — 묶음의 한 줄이 되는 순간 "여러 링크 중 하나"로 읽힙니다.
//   구독권은 안 해도 그만인 덤이라 맨 아래에 따로 둡니다.
export const PLATFORM_LINK = {
  title: '해커톤 플랫폼',
  desc: '제출 · 공지 · 우리 팀 페이지',
  url: 'https://www.52g-hackathon.com/',
}

// '더 관심이 있다면?' 아래에 놓이는, 해도 되고 안 해도 되는 것들.
// 늘어날 수 있어 배열로 둡니다.
export const EXTRA_LINKS = [
  {
    ...
    title: '더밀크 구독권',
    desc: '참가자 대상 구독권 신청',
    url: 'https://themiilk-event2026.web.app/gs-hackathon/',
  },
]
...
// 1-4. 해커톤 Wi-Fi
//
//   현장 곳곳에 구역별 AP가 따로 있고 비밀번호도 다릅니다. 안내 포스터가
...
//
//   ⚠️ 여기 적힌 비밀번호는 참가자 앱 번들에 그대로 들어갑니다. 현장
//      포스터에 붙는 것과 같은 값이라 그 이상 숨길 것은 없지만, 해커톤이 끝나면
//      AP 비밀번호를 바꾸는 편이 좋습니다.
// ---------------------------------------------------------------
// 구역 배치도. 앱은 "우리 구역 비밀번호"를 이미 알려주지만, 라운지로
// 옮겨 앉거나 자리를 못 찾을 때 "어느 구역이 어디인가"는 알 길이 없었습니다.
// 그 한 가지를 그림이 맡습니다.
//
//   src  : 팀 정보 시트에 그대로 펼쳐 보이는 배치도. 원본 포스터에서 지도만
//          잘랐습니다 — 아래쪽 '구역별 P/W' 목록은 이 앱이 이미 우리 구역만
//          골라 보여주고 있어, 시트에서 두 번 읽을 이유가 없습니다.
//   full : 눌렀을 때 새 탭에서 열리는 포스터 전체(비밀번호 목록 포함).
export const WIFI_MAP = {
  src: './board/wifi-map.jpg?v=1',
  full: './board/wifi-full.jpg?v=1',
  alt:
    '해커톤 Wi-Fi 구역 배치도 — Act Site 필드리그 HoL_01~06, ' +
    'Build Site 개발자리그 HoL_07~08, 라운지 HoL_Lounge_01~03',
}
...
export const TEAMS = {
  ...
  'E-02': { name: "Grill Me", affiliation: "GS에너지", size: 3 },
  'E-03': { name: "오디세이", affiliation: "보령LNG터미널", size: 2 },
  'E-04': { name: "CTRL + AI", affiliation: "위드인천에너지", size: 2 },
  'E-05': { name: "리투(Lee Two)", affiliation: "인천종합에너지", size: 2 },
  'E-06': { name: "KJ", affiliation: "인천종합에너지", size: 2 },
  'E-07': { name: "안녕하세요차지비입니다잘부탁드립니다.", affiliation: "GS차지비", size: 3 },
  'E-08': { name: "PLAI, 차지", affiliation: "GS차지비", size: 3 },
  'E-09': { name: "당진파파", affiliation: "GS EPS", size: 3 },
  'E-10': { name: "빌드업", affiliation: "GS EPS", size: 3 },
  'E-11': { name: "스마트스퀘어", affiliation: "GS EPS", size: 2 },
  'E-12': { name: "스마트터빈", affiliation: "GS EPS", size: 3 },
  'E-13': { name: "파도", affiliation: "GS EPS", size: 2 },
  'E-14': { name: "HOPE", affiliation: "GS EPS", size: 4 },
  'E-15': { name: "Jun & Kun", affiliation: "GS EPS", size: 2 },
  'E-16': { name: "고객운영팀", affiliation: "GS파워", size: 3 },
  'E-17': { name: "서든어택", affiliation: "GS파워", size: 4 },
  'E-18': { name: "성능주의", affiliation: "GS파워", size: 4 },
  'E-19': { name: "파울링 헌터스", affiliation: "GS파워", size: 4 },
  'E-20': { name: "20W50", affiliation: "GS칼텍스", size: 3 },
  'E-21': { name: "시고시고", affiliation: "GS칼텍스", size: 3 },
  'E-22': { name: "아장아장", affiliation: "GS칼텍스", size: 4 },
  'E-23': { name: "오뒷에이아이야", affiliation: "GS칼텍스", size: 3 },
  'E-24': { name: "지칼드림팀", affiliation: "GS칼텍스", size: 3 },
  'E-25': { name: "체크메이트", affiliation: "GS칼텍스", size: 3 },
  'E-26': { name: "컴플AI언스", affiliation: "GS칼텍스", size: 3 },
  'E-27': { name: "Bid-Tamin", affiliation: "GS칼텍스", size: 2 },
  'E-28': { name: "Fixipedia", affiliation: "GS칼텍스", size: 4 },
  'E-29': { name: "MOC MATE", affiliation: "GS칼텍스", size: 3 },
  'E-30': { name: "구트레인", affiliation: "GS칼텍스", size: 4 },
  'E-31': { name: "RA", affiliation: "GS칼텍스", size: 4 },
  'E-32': { name: "AIDEAL (에이디얼)", affiliation: "GS건설", size: 4 },
  'E-33': { name: "기사회생", affiliation: "GS건설", size: 4 },
  'E-34': { name: "보건보건", affiliation: "GS건설", size: 3 },
  'E-35': { name: "AIgnition", affiliation: "GS건설", size: 4 },
  'E-36': { name: "BID ONE(비드원)", affiliation: "GS건설", size: 4 },
  'E-37': { name: "BS", affiliation: "GS건설", size: 4 },
  'E-38': { name: "FanTAXtic", affiliation: "GS건설", size: 2 },
  'E-39': { name: "GS 공구리에이티브", affiliation: "GS건설", size: 4 },
  'E-40': { name: "MU", affiliation: "GS건설", size: 2 },
  'E-41': { name: "Noise Huntrix", affiliation: "GS건설", size: 3 },
  'E-42': { name: "XI-Nergy", affiliation: "자이C&A", size: 3 },
  'E-43': { name: "NARASEE", affiliation: "자이S&D", size: 3 },
  'E-44': { name: "Xisafety", affiliation: "자이S&D", size: 3 },
  'E-45': { name: "3삼5오", affiliation: "GS리테일", size: 4 },
  'E-46': { name: "강동어겐", affiliation: "GS리테일", size: 4 },
  'E-47': { name: "김이조장", affiliation: "GS리테일", size: 4 },
  'E-48': { name: "딱걸렸4", affiliation: "GS리테일", size: 4 },
  'E-49': { name: "리스크 제로", affiliation: "GS리테일", size: 4 },
  'E-50': { name: "미녀사총사", affiliation: "GS리테일", size: 4 },
  'E-51': { name: "불티를깨워", affiliation: "GS리테일", size: 2 },
  'E-52': { name: "블랙핑크", affiliation: "GS리테일", size: 4 },
  'E-53': { name: "뽀삐", affiliation: "GS리테일", size: 2 },
  'E-54': { name: "세끼통역", affiliation: "GS리테일", size: 3 },
  'E-55': { name: "수퍼프롬프트", affiliation: "GS리테일", size: 2 },
  'E-56': { name: "신선강화B", affiliation: "GS리테일", size: 4 },
  'E-57': { name: "신선강화C", affiliation: "GS리테일", size: 3 },
  'E-58': { name: "안AI잘부", affiliation: "GS리테일", size: 4 },
  'E-59': { name: "온앤오프", affiliation: "GS리테일", size: 3 },
  'E-60': { name: "우연이연", affiliation: "GS리테일", size: 4 },
  'E-61': { name: "커넥트 you & me", affiliation: "GS리테일", size: 3 },
  'E-62': { name: "커타고", affiliation: "GS리테일", size: 4 },
  'E-63': { name: "햄샘수", affiliation: "GS리테일", size: 3 },
  'E-64': { name: "홈동이들", affiliation: "GS리테일", size: 4 },
  'E-65': { name: "효리수", affiliation: "GS리테일", size: 4 },
  'E-66': { name: "Audit Say (오디세이)", affiliation: "GS리테일", size: 4 },
  'E-67': { name: "GiveS", affiliation: "GS리테일", size: 4 },
  'E-68': { name: "HiAi", affiliation: "GS리테일", size: 4 },
  'E-69': { name: "HLH컴퍼니", affiliation: "GS리테일", size: 3 },
  'E-70': { name: "SO, GOOD", affiliation: "GS리테일", size: 3 },
  'E-71': { name: "Team Argo", affiliation: "GS리테일", size: 4 },
  'E-72': { name: "Team Hermes", affiliation: "GS리테일", size: 4 },
  'E-73': { name: "VOC세편살", affiliation: "GS리테일", size: 3 },
  'E-74': { name: "끝까지 버팀", affiliation: "GS네트웍스", size: 3 },
  'E-75': { name: "구미 야호", affiliation: "GS구미열병합발전", size: 2 },
  'E-76': { name: "FC동해", affiliation: "GS동해전력", size: 2 },
  'E-77': { name: "위험한팀", affiliation: "파르나스 호텔", size: 2 },
  'E-78': { name: "공하Z", affiliation: "파르나스 호텔", size: 2 },
  'E-79': { name: "미철즈", affiliation: "파르나스 호텔", size: 2 },
  'E-80': { name: "신림파", affiliation: "파르나스 호텔", size: 3 },
  'E-81': { name: "DNA (Dining Needs Analyzer)", affiliation: "파르나스 호텔", size: 4 },
  'E-82': { name: "JOEvolution", affiliation: "파르나스 호텔", size: 3 },
  'E-83': { name: "JSS", affiliation: "파르나스 호텔", size: 4 },
  'E-84': { name: "W/IRD", affiliation: "파르나스 호텔", size: 2 },
  'E-85': { name: "쉬거라", affiliation: "GS글로벌", size: 4 },
  'E-86': { name: "용쑤형", affiliation: "GS글로벌", size: 4 },
  'E-87': { name: "지글 지글 식사팀", affiliation: "GS글로벌", size: 4 },
  'E-88': { name: "추심단", affiliation: "GS글로벌", size: 4 },
  'E-89': { name: "AI고 뭐고 일단 해", affiliation: "GS글로벌", size: 4 },
  'E-90': { name: "GSG26", affiliation: "GS글로벌", size: 3 },
  'E-91': { name: "Market Radar", affiliation: "GS글로벌", size: 3 },
  'E-92': { name: "MAY THE MISO BE WITH YOU", affiliation: "GS글로벌", size: 2 },
  'E-93': { name: "Zero Shot", affiliation: "GS글로벌", size: 4 },
  'E-94': { name: "잘찾조", affiliation: "GS엔텍", size: 2 },
  'E-95': { name: "서울의별", affiliation: "GS스포츠", size: 3 },
  'E-96': { name: "에푸씨", affiliation: "GS스포츠", size: 2 },
  'E-97': { name: "A-01", affiliation: "계열사 혼합", size: 4 },
  'E-98': { name: "A-02", affiliation: "계열사 혼합", size: 3 },
  'E-99': { name: "A-03", affiliation: "계열사 혼합", size: 3 },
  'E-100': { name: "A-04", affiliation: "계열사 혼합", size: 4 },
  'E-101': { name: "A-05", affiliation: "계열사 혼합", size: 3 },
  'E-102': { name: "A-06", affiliation: "계열사 혼합", size: 4 },
  'E-103': { name: "A-07", affiliation: "계열사 혼합", size: 4 },
  'E-104': { name: "A-08", affiliation: "계열사 혼합", size: 4 },
  // 개발자리그 G-05에 있다가 필드리그로 옮겨온 팀입니다
  'E-105': { name: "써브(Thermal Bridge)", affiliation: "GS글로벌, GS파워", size: 2 },
  // 배정표에서 뒤늦게 나온 GS건설 팀입니다
  'E-106': { name: "일단해보자", affiliation: "GS건설", size: 3 },

  // 외부사 — 자리배치표에는 자리가 없고 번호만 시트에 적혀 있습니다.
  // affiliation(소속)은 호출 기록을 구글 시트에 쌓을 때 쓰는 값입니다.
  //   계열사 팀   그 계열사 이름 (GS EPS · GS리테일 …)
  //   계열사 혼합 개인 지원자끼리 꾸린 팀(A-01~A-08) — 회사가 여럿이라 한
  //               이름으로 적을 수 없습니다.
  //   외부사/관계사  계열사가 아닌 곳. 회사 이름은 아래 company 에 있고,
  //               팀명 자리에 그대로 나옵니다.
  // company 와 역할이 다릅니다 — company 는 "팀명이 없을 때 대신 보여줄
  // 이름"이고, affiliation 은 "어디 소속인가"입니다.
  // 팀명이 아직 없어 회사명으로 확인시켜 줍니다(company). 정해지면 name을 채우세요.
  'E-200': { name: "오리온 1팀", affiliation: "외부사/관계사", company: "오리온", size: 4 },
  'E-201': { name: "오잉메리호", affiliation: "외부사/관계사", company: "오리온", size: 4 },
  'E-202': { name: "오리온 3팀", affiliation: "외부사/관계사", company: "오리온", size: 4 },
...
// ---------------------------------------------------------------
// 4. 알레르기 선택지
//    ⚠️ 주문 메뉴(MENUS)나 그냥 나오는 끼니(SERVED_MEALS)에 실제로 들어간
//      성분만 선택지로 둡니다. 어디에도 없는 성분을 늘어놓으면 현장에서
//      쓸 데 없는 응답이 쌓이기 때문입니다.
//      → 메뉴가 바뀌면 이 목록도 함께 갱신하세요.
//    ※ 먹을 것을 미리 정해두는 게 아닙니다. 여기서 모은 현황을 보고
//      식음 운영 크루가 찾아가 무엇을 드실 수 있는지 여쭙습니다 —
//      알레르기를 알려주신 것이지 그 끼니를 드시겠다고 하신 것은
//      아니라서, 수를 미리 확정할 수 없습니다.
//    ※ 저장 형태: team.allergies = [[사람1의 알레르기...], [사람2의 알레르기...]]
//      (사람 단위로 배열을 나눠 저장 — 1명이 여러 개인지 여러 명이 각각
//      하나씩인지에 따라 찾아갈 사람과 도시락 조합이 달라지기 때문)
//    ※ 이 표기는 MENUS의 allergens와 정확히 일치해야 합니다. 그래야
//      "이 사람이 어떤 메뉴를 먹을 수 있는지"(아래 personDiet/teamDiet)와
//      관리자 화면의 찾아갈 명단이 동작합니다.
// ---------------------------------------------------------------
// 앞의 다섯은 야식·아침(주문 메뉴)에 들어 있어 아래 personDiet/teamDiet 판정을
// 바꿉니다. 갑각류는 주문 메뉴에 없어 그 판정에는 걸리지 않지만, 점심 도시락에
// 새우가 들어가 받아둬야 합니다 — 도시락(저녁·점심)에서 재료만 빼면 되는 것은
// 아래 CATERING_SWAPS 가 따로 세어 관리자 화면으로 보냅니다.
export const ALLERGY_OPTIONS = ['우유', '밀', '돼지고기', '쇠고기', '토마토', '갑각류(새우)']

// 케이터링 끼니(도시락)에서 메뉴 몇 가지를 빼면 되는 경우.
//
// ⚠️ item 은 SERVED_MEALS 의 그 끼니 items 를 보고 손으로 맞춰 둔 표입니다.
//    호텔이 구성을 바꾸면 저절로 따라오지 않으니 함께 고치세요. 실제로 무엇이
//    들어 있는지는 호텔에 확인하는 것이 맞습니다 — 여기 적힌 것은 '알려진
//    것'이지 성분표가 아닙니다.
//
// 야식·아침은 G-Order로 주문하니 personDiet/teamDiet 가 '이 사람이 먹을 수
// 있는 메뉴'를 계산해 찾아갈 명단을 뽑아냅니다. 도시락은 주문 대상이
// 아니라 그 계산에 아예 잡히지 않고, 찾아갈 일도 아닙니다 —
// 도시락에서 그 재료만 뺀 버전을 그 팀에 주면 끝입니다.
//
// 그래서 필요한 정보가 '몇 인분'이 아니라 '어느 팀에 몇 개'입니다. 배부하는
// 사람이 팀 번호를 보고 다른 도시락을 건네야 하니까요.
//
// 알레르기별이 아니라 끼니별로 묶습니다. 새우와 토마토를 둘 다 못 먹는
// 사람에게 도시락 두 개를 주는 게 아니라, 둘 다 뺀 도시락 하나를 주니까요.
//   mealLabel : 어느 끼니인지 (SERVED_MEALS의 label과 같게)
//   eatAt     : 먹는 시각. 등록 화면에서 주문 끼니와 한 줄기로 세워
//               시간 순서대로 보여주는 데 씁니다 (MEALS.eatAt 과 같은 형식)
//   exclusions : ALLERGY_OPTIONS 순서대로 적습니다 — 여러 개가 걸린 사람의
//               안내 문구('돼지고기 · 토마토')가 칩과 같은 차례로 나옵니다
//   exclusions.allergy : ALLERGY_OPTIONS 의 표기와 글자 그대로 같아야 합니다
//   exclusions.label   : 화면에 짧게 찍히는 이름 ('새우 제외')
//   exclusions.item    : 어느 메뉴 때문인지 적어두는 기록. 화면에는 안 나갑니다
//               (주방이 도시락을 아니, 무엇을 빼라고 일러줄 필요가 없습니다).
//               SERVED_MEALS 의 그 끼니 items 에 적힌 이름 그대로 두세요 —
//               메뉴가 바뀌었을 때 이 표가 낡았는지 대조하는 유일한 단서입니다
export const CATERING_SWAPS = [
  {
    id: 'day1-dinner',
    icon: '🍱',
    mealLabel: '[DAY 1] 저녁',
    mealDesc: '파르나스 호텔 도시락 (한식)',
    eatAt: '2026-09-21T18:00:00',
    exclusions: [
      { allergy: '우유', label: '우유', item: '당근 케이크' },
      { allergy: '밀', label: '밀', item: '당근 케이크 · 생선전' },
      { allergy: '쇠고기', label: '쇠고기', item: '차돌 묵은지 볶음 · 갈비살 구이(소고기)' },
    ],
  },
  {
    id: 'day2-lunch',
    icon: '🍱',
    mealLabel: '[DAY 2] 점심',
    mealDesc: '파르나스 호텔 도시락 (양식)',
    eatAt: '2026-09-22T12:00:00',
    exclusions: [
      {
        allergy: '우유',
        label: '우유',
        // 열두 가지 중 다섯. 재료 하나 빼는 정도가 아니라서, 호텔과 한 번
        // 맞춰보고 아예 다른 구성으로 갈지 정하는 편이 낫습니다.
        item: '얼그레이 치즈 케이크 · 로메인 시저 샐러드 · 컬러풀 토마토, 치즈 · 허브 버터 새우 · 양송이 수프',
      },
      {
        allergy: '밀',
        label: '밀',
        item: '얼그레이 치즈 케이크 · 버섯 크러스트 햄버거 스테이크와 감자 구이(소고기) · 허브크러스트 가자미 · 양송이 수프',
      },
      { allergy: '돼지고기', label: '돼지고기', item: '소시지' },
      { allergy: '쇠고기', label: '쇠고기', item: '버섯 크러스트 햄버거 스테이크와 감자 구이(소고기)' },
      { allergy: '토마토', label: '토마토', item: '컬러풀 토마토, 치즈' },
      { allergy: '갑각류(새우)', label: '새우', item: '허브 버터 새우' },
    ],
  },
]

// 한 사람이 이 끼니에서 빼야 하는 재료들. 해당 없으면 빈 배열입니다.
export function cateringSwapFor(swap, allergies) {
  const mine = allergies || []
  return (swap.exclusions || []).filter((e) => mine.includes(e.allergy))
}

// 주방이 만들 '도시락 종류'별로 묶습니다.
//
// 팀 단위로 늘어놓으면 배부는 되지만 조리가 안 됩니다 — 돼지고기를 뺀 것과
// 새우를 뺀 것은 아예 다른 도시락인데 한 줄에 섞여 있으니까요. 그래서 뺄
// 재료 조합이 같은 것끼리 한 덩이로 묶고, 그 아래에 갈 팀을 답니다.
//
// 묶는 기준은 '한 사람이 빼야 하는 재료 전부'입니다. 돼지고기와 토마토를 둘
// 다 못 먹는 사람에게는 둘 다 뺀 도시락 하나가 나가지, 돼지고기 뺀 것과
// 토마토 뺀 것이 따로 가지 않습니다. 그래서 종류별 개수를 더하면 총 개수와
// 정확히 맞습니다.
//
//   teams : { [teamId]: { allergies: [[...], [...]] } } (서버 스냅샷 그대로)
//   variants : [{ key, label: '돼지고기, 토마토', count, teams: [{ teamId, count }] }]
export function cateringSwapCounts(teams, swap) {
  const byVariant = new Map()
  let total = 0
  Object.entries(teams || {}).forEach(([teamId, team]) => {
    const people = (team?.allergies || []).map((p) => (Array.isArray(p) ? p : [p]))
    people.forEach((person) => {
      const removed = cateringSwapFor(swap, person)
      if (!removed.length) return
      total += 1
      // CATERING_SWAPS에 적은 차례를 그대로 씁니다 — 같은 조합이 사람마다
      // 다른 순서로 적히면 한 덩이로 묶이지 않습니다.
      const key = removed.map((e) => e.allergy).join('|')
      let v = byVariant.get(key)
      if (!v) {
        v = {
          key,
          label: removed.map((e) => e.label).join(', '),
          count: 0,
          teams: new Map(),
        }
        byVariant.set(key, v)
      ...
      v.count += 1
      v.teams.set(teamId, (v.teams.get(teamId) || 0) + 1)
    })
  })

  const variants = [...byVariant.values()]
    // 많이 나가는 것부터. 같은 수면 이름순이라 화면이 흔들리지 않습니다.
    .sort((a, b) => b.count - a.count || a.label.localeCompare(b.label))
    .map((v) => ({
      ...v,
      teams: [...v.teams.entries()]
...
//    ※ 알레르기 판정(personDiet/teamDiet)에는 쓰이지 않습니다.
...
//     - 우유·밀·돼지고기 중 하나라도 있으면 → 4개 메뉴 전부 불가(두 끼 모두)
...
//   - altByMeal     : 끼니별로 "드실 수 있는 메뉴가 하나도 없는 인원 수"
//   - altPeople     : 그 인원의 알레르기 목록 (크루가 찾아갈 때 참고)
...
//
//   타임테이블은 리그마다 이미지가 다릅니다. 여기서는 필드리그 것(board)
//   에서 이름·아이콘만 빌려 쓰는데, 두 장의 label·icon 이 같아서 어느
//   리그에서 열어도 안내에는 똑같이 '타임테이블' 한 칸으로 섭니다.
